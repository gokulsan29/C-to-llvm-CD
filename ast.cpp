...
list_n<T_NODE>::to_string(string prefix) const
{
  ...
  for (int i = 0;i < l.size();i++) {
    ...
    ret += prefix + elem_prefix + l[i]->to_string(prefix + child_prefix);
  }
  return ret;
} 

string
declaration_specifier_n::to_string(string prefix) const
{
...
string
declaration_specifiers_n::to_string(string prefix) const
{
  ...
  for(int i = 0;i < decl_spec_list.size();i++) {
    ret += decl_spec_list[i]->to_string(prefix);
...
external_declaration_n::to_string(string prefix) const
{
  if (this->m_function_definition) {
    return this->m_function_definition->to_string(prefix);
  }
  assert(this->m_declaration);
  return this->m_declaration->to_string(prefix);
}
...
function_definition_n::to_string(string prefix) const
{
  ...
  ret += prefix + this->m_declaration_specifiers->to_string("|-");
  return ret;
}
